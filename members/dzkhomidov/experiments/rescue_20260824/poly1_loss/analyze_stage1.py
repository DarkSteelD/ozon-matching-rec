...
ROOT = Path("/home/dzkhomidov/matching-work/rescue_20260824/poly1_loss")
HARD = Path("/home/dzkhomidov/matching-work/data/hand_pairs.parquet")
FOLDS = ("fold_01", "fold_02")
VARIANTS = ("bce", "poly05", "polyneg05")
...
def main():
    ...
    summary = pivot.groupby("category", as_index=False).agg(
        rows=("bce", "size"),
        bce_mean=("bce", "mean"),
        poly05_delta_mean=("delta_poly05", "mean"),
        poly05_delta_min=("delta_poly05", "min"),
        poly05_delta_max=("delta_poly05", "max"),
        polyneg05_delta_mean=("delta_polyneg05", "mean"),
    )
    summary["poly05_same_positive"] = summary.poly05_delta_min > 0
    summary["poly05_same_negative"] = summary.poly05_delta_max < 0
    summary.sort_values("poly05_delta_mean", ascending=False).to_csv(
        ROOT / "stage1_score" / "category_summary.csv", index=False
    )
    print(thresholds.to_string(index=False))
    print(summary.sort_values("poly05_delta_mean", ascending=False).to_string(index=False))
