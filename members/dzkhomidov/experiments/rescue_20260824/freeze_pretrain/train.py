...
def run_fold(args, fold, df, ids, tt, ids_r, tt_r, tok):
    ...
    frozen = range(6) if args.freeze == "bottom6" else range(6, 12) if args.freeze == "top6" else ()
    for i in frozen:
        for parameter in layers[i].parameters():
            parameter.requires_grad = False
    assert all(not p.requires_grad for i in frozen for p in layers[i].parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())
    print(f"{fold} freeze={args.freeze} trainable={trainable}/{total} ({trainable/total:.3%})", flush=True)
    ...
    meta = {"variant": args.variant, "fold": fold, "steps": total_steps,
            ...
            "max_len": args.max_len, "freeze": args.freeze,
...
def main():
    ...
    ap.add_argument("--freeze", choices=["none", "bottom6", "top6"], default="none")
    args = ap.parse_args()
