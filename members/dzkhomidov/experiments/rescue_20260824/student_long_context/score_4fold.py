...
result = {"rows": rows,
          "means": {v: {k: float(np.mean([r[k] for r in rows if r["variant"] == v]))
                         for k in ("macro_ap", "pooled_ap", "delta_macro", "delta_pooled")}
                    for v in ("len224", "len384")}}
(root / "metrics_4fold.json").write_text(json.dumps(result, ensure_ascii=False, indent=2))
pl.DataFrame(cats).write_csv(root / "category_metrics_4fold.csv")
