...
def set_determinism(seed: int):
    """Reset every training RNG before each paired arm."""
    random.seed(seed)
    np.random.seed(seed)
...
def train_fold(args, fold, variant, weight, selected, df, ids, tt, target, tokenizer):
    ...
    fold_seed = args.seed + int(fold[-2:])
    ...
    rng = np.random.default_rng(fold_seed); started = time.time(); step = 0; model.train()
    ...
    (out / f"{fold}.meta.json").write_text(json.dumps({"variant": variant, "fold": fold,
        ...
        "fold_seed": fold_seed, "torch_initial_seed": torch.initial_seed(),
...
def main():
    ...
    ap.add_argument("--folds", default="fold_01,fold_02"); ap.add_argument("--variants", default="baseline,hard050")
    ...
    for fold in args.folds.split(","):
        ...
        for variant in args.variants.split(","):
            if variant.startswith("baseline"): selected, weight = np.zeros(df.height, bool), 1.0
            else:
