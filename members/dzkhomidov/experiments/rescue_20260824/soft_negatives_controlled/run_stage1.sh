...
nvidia-smi -i "$gpu" \
  ...
  --format=csv,noheader > "$here/preflight_stage1.log"
...
"$py" "$here/train_controlled.py" \
  ...
  --folds fold_01,fold_02 --variants baseline,hard050,baseline_repeat \
  2>&1 | tee "$here/train_stage1.log"
mv "$here/run_manifest.json" "$here/run_manifest_stage1.json"

"$py" "$here/score.py" \
  --data "$data" --root "$here/preds" --archived-baseline "$oof" \
  --folds fold_01,fold_02 \
  --variants baseline_rerun,hard050,baseline_repeat \
  --output "$here/metrics_stage1.json" 2>&1 | tee "$here/score_stage1.log"

"$py" - "$here/metrics_stage1.json" > "$here/gate_stage1.txt" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
rows = [x for x in d["rows"] if x["variant"] == "hard050"]
passed = len(rows) == 2 and all(x["delta_macro"] > 0.001 for x in rows)
for x in rows:
    print(x["fold"], f'{x["delta_macro"]:+.9f}')
print("gate_pass", passed)
