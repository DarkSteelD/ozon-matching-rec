"""E-CUP 2026 matching: кросс-энкодер RuModernBERT-base, усреднение весов по фолдам.
...
*   **Бленд предсказаний четырёх фолдовых моделей.** Каждая модель считает свои
    вероятности, они переводятся в ранги и усредняются. Дороже супа вчетверо, но
    честно измеренный выигрыш усреднения предсказаний на этом семействе моделей
    составил +0.001854 к среднему и +0.002024 к макро, на 4 фолдах из 4.
    Защита по времени пропускает хвостовые модели, если бюджет на исходе:
    бленд из трёх лучше, чем отсутствие ответа.
...
T0 = time.time()
TIME_BUDGET = 15.5 * 60  # запас внутри 20-минутного лимита
ATTRS_LIMIT = 800
...
def main() -> None:
    ...
    left, right, penalty = build_prio_texts(matches, id2name, id2cat, id2attr)
    print(f"{len(left)} пар, тексты собраны, прошло {elapsed():.0f}с", flush=True)
    ...
    for number, spec in enumerate(specs):
        ...
        if number > 0 and model_cost is not None \
                and elapsed() + model_cost * 1.1 > TIME_BUDGET:
            print(f"защита по времени: пропускаю {spec['path']} "
                  f"(прошло {elapsed():.0f}с)", flush=True)
            continue
        started_model = time.time()
        predicted = score(spec, left, right, device)
        model_cost = time.time() - started_model
