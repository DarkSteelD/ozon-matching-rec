...
def main() -> None:
    ...
    # Штраф за размер НАМЕРЕННО отключён. Он придуман для моделей, которые
    # размерных атрибутов не видят: у дообученных они выброшены из текста
    # (DROP_FASHION), и правило возвращает информацию обратно множителем. Но
    # претрен учился на текстах, где размеры есть — проверено, 58 из 1000
    # текстов их содержат. Ему поправка может быть не нужна или вредна.
    print(f"штраф за размер отключён; он сработал бы на "
          f"{int(np.array(penalty).sum())} парах", flush=True)
