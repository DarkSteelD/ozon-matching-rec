...
QWEN_SYSTEM = ("Judge whether the Document meets the requirements based on the "
               "Query and the Instruct provided. Note that the answer can only "
               "be \"yes\" or \"no\".")
QWEN_INSTRUCT = ("Given two product cards from an online marketplace, judge "
                 "whether they describe the exact same product.")
QWEN_PREFIX = ("<|im_start|>system\n" + QWEN_SYSTEM + "<|im_end|>\n"
               "<|im_start|>user\n<Instruct>: " + QWEN_INSTRUCT + "\n")
QWEN_SUFFIX = "<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n\n"


def score_qwen(spec, left, right, pair_cats, device, deadline=None):
    """Каузальный реранкер: p(yes) через разность логитов, TTA двумя проходами.

    Пары обрабатываются группами по категориям в порядке spec["cat_priority"];
    если по темпу не успеваем — оставшиеся группы получают NaN, и роутинг сам
    откатывает их на действующие ветки (fail-closed по построению).
    """
    from transformers import AutoModelForCausalLM
    tokenizer = AutoTokenizer.from_pretrained(spec["path"], padding_side="left")
    tokenizer.truncation_side = "left"
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    try:
        model = AutoModelForCausalLM.from_pretrained(
            spec["path"], torch_dtype=torch.bfloat16,
            attn_implementation="sdpa")
    except TypeError:
        model = AutoModelForCausalLM.from_pretrained(
            spec["path"], dtype=torch.bfloat16, attn_implementation="sdpa")
    model.to(device).eval()
    model.config.use_cache = False
    yes_id = tokenizer.convert_tokens_to_ids("yes")
    no_id = tokenizer.convert_tokens_to_ids("no")
    budget = spec.get("char_budget", 380)
    max_len = spec.get("max_len", 384)
    batch = spec.get("batch", 96) if device.type == "cuda" else 8
    total = len(left)
    out = np.full(total, np.nan)
    tta = bool(spec.get("tta_swap"))
    rate = spec.get("rate_guess", 200.0)

    def one_pass(idx, a_side, b_side):
        vals = np.zeros(len(idx))
        pos = {p: j for j, p in enumerate(idx)}
        rough = np.array([len(a_side[i]) + len(b_side[i]) for i in idx])
        order = [idx[k] for k in np.argsort(rough, kind="stable")]
        with torch.inference_mode():
            for s in range(0, len(order), batch):
                pick = order[s:s + batch]
                prompts = [QWEN_PREFIX + "<Query>: " + a_side[i][:budget]
                           + "\n<Document>: " + b_side[i][:budget] + QWEN_SUFFIX
                           for i in pick]
                enc = tokenizer(prompts, padding=True, pad_to_multiple_of=32,
                                truncation=True, max_length=max_len,
                                return_tensors="pt").to(device)
                with torch.autocast("cuda", dtype=torch.bfloat16,
                                    enabled=device.type == "cuda"):
                    try:
                        lo = model(**enc, logits_to_keep=1).logits[:, -1, :]
                    except TypeError:
                        lo = model(**enc).logits[:, -1, :]
                diff = (lo[:, yes_id] - lo[:, no_id]).float().cpu().numpy()
                probs = 1.0 / (1.0 + np.exp(-diff))
                for i, p in zip(pick, probs):
                    vals[pos[i]] = p
        return vals

    cats_arr = np.asarray(pair_cats)
    priority = spec.get("cat_priority") or sorted(set(cats_arr.tolist()))
    for cat in priority:
        group = np.flatnonzero(cats_arr == cat)
        if not len(group):
            continue
        passes = 2 if tta else 1
        need = passes * len(group) / max(rate, 1.0)
        if deadline is not None and elapsed() + need > deadline:
            print(f"  qwen: {cat} ({len(group)} пар) не влезает "
                  f"(нужно {need:.0f}с) — откат на действующие ветки",
                  flush=True)
            continue
        t_grp = time.time()
        p1 = one_pass(group, left, right)
        p2 = one_pass(group, right, left) if tta else p1
        out[group] = (p1 + p2) / 2.0
        took = time.time() - t_grp
        rate = passes * len(group) / max(took, 1e-6)
        print(f"  qwen: {cat} {len(group)} пар за {took:.0f}с "
              f"({rate:.0f} пар/с c TTA={tta})", flush=True)
    del model
    if device.type == "cuda":
...
def main() -> None:
    ...
    for number, spec in enumerate(specs):
        ...
        if spec.get("scorer") == "qwen_yesno":
            part = score_qwen(spec, [source_left[i] for i in index],
                              [source_right[i] for i in index],
                              [pair_category[i] for i in index], device,
                              deadline)
        else:
